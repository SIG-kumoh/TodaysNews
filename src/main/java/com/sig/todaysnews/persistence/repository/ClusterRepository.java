...
public interface ClusterRepository extends JpaRepository<Cluster, Long> {
    @Query("SELECT c FROM Cluster c WHERE c.section.sectionId = :sid AND FUNCTION('DATE', c.regdate) = :date ORDER BY c.size DESC")
    List<Cluster> findClustersBySidAndDate(@Param("sid") Long sid, @Param("date") LocalDate date);

    @Query("SELECT c FROM Cluster c WHERE FUNCTION('DATE', c.regdate) = :date AND c.relatedCluster in :relatedClusters ORDER BY c.size DESC")
    ...
    @Query("SELECT FUNCTION('count', c) FROM Cluster c WHERE c.section.sectionId = :sid AND FUNCTION('DATE', c.regdate) = :date")
